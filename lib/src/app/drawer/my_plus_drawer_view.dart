...
class _MyPlusDrawerState extends ViewState<MyPlusDrawer, MyPlusDrawerController> {
        ...
        children: [
          FittedBox(
            ...
            child: IconButton(
              ...
              }, icon: Image.asset('assets/images/sidebar_login.png', color: theme.appBarTheme.foregroundColor,),
              iconSize: 55.0,
            ),
          ),
  ...
  // hot key
  List<Widget> _buildRemainingHotKeysItems(BuildContext context, List<MenuSettingItem> hotKeys) {
  ...
  List<Widget> _buildHotKeysItems(BuildContext context, List<MenuSettingItem> hotKeys) {
            ...
            SizedBox(
              ...
              child: TextButton(onPressed: ()=> {
                ...
                style: ButtonStyle(
                  ...
                  backgroundColor: MaterialStateProperty.all(theme.appBarTheme.backgroundColor),
                ),
              ),
            ),
            (hotKeys.length > 1) 
  ...
  _onFooterExpansion(bool expanding, int index, ) {
    if (expanding) {
      setState(() {
  ...
  Widget _buildFooterItem(MenuSettingItem item, int index) {
    ...
    return ExpansionTile(
      ...
      childrenPadding: const EdgeInsets.only(left: 10),
      onExpansionChanged: (bool expanding)=> _onFooterExpansion(expanding, index),
      children: item.children.asMap().entries.map((e) => _buildFooterItem(e.value, e.key)).toList(),
    );
  ...
  _onMenuExpansion(bool expanding, int index, ) {
    if (expanding) {
      setState(() {
        const Duration(seconds: 0);
  ...
  Widget _buildMenuItem(MenuSettingItem item, int index) {
    ...
    return ExpansionTile(
      ...
      childrenPadding: const EdgeInsets.only(left: 10),
      onExpansionChanged: (bool expanding)=> _onMenuExpansion(expanding, index),
      children: item.children.asMap().entries.map((e) => _buildMenuItem(e.value, e.key)).toList(),
    );
  ...
  // sections
  List<String> _buildSections(List<MenuSettingItem> hotKeys, List<MenuSettingSwitchItem> switchs) {
    ...
    return ControlledWidgetBuilder<MyPlusDrawerController>(
      builder: (context, controller) {
        ...
        if (controller.siteSetting != null) {
          ...
          List<MenuSettingSwitchItem> switchs = List.from(controller.siteSetting!.menu.switchs.where((element) => element.enabled));
          ...
          return Drawer(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                switch (element) {
                  case 'header': return _buildDrawerHeader(context);
                  case 'hot_key': return _buildHotKeys(context, hotKeys);
                  case 'footer': return _buildFooter(context, footer);
                  case 'news': return _buildEvent(context, eventList);
                  case 'menu': return _buildMenu(context, header);
                  case 'category': return _buildCategory(context, sidebar);
                  case 'info': return _buildInfo(theme, sections.contains('footer'), basic);
