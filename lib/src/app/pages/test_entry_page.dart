...
class _TestPageState extends State<TestPage> {
  ...
  Widget build(BuildContext context) => Padding(
      ...
      child: SingleChildScrollView(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const ErrorText('訊息過長會自動換行，這只是一個加了三角型圖案的紅色文字，這只是一個加了三角型圖案的紅色文字，這只是一個加了三角型圖案的紅色文字'),
          const ErrorText('限制一行，這只是一個加了三角型圖案的紅色文字，這只是一個加了三角型圖案的紅色文字，這只是一個加了三角型圖案的紅色文字',
              maxLines: 1),
          const ErrorText(
            '限制二行，這只是一個加了三角型圖案的紅色文字，這只是一個加了三角型圖案的紅色文字，這只是一個加了三角型圖案的紅色文字',
            maxLines: 2,
          ),
          const ErrorText('一行字尾漸淡，這只是一個加了三角型圖案的紅色文字，這只是一個加了三角型圖案的紅色文字，這只是一個加了三角型圖案的紅色文字',
              maxLines: 1, overflow: TextOverflow.fade),
          const ErrorText('多行字尾漸淡，這只是一個加了三角型圖案的紅色文字，這只是一個加了三角型圖案的紅色文字，這只是一個加了三角型圖案的紅色文字',
              maxLines: 2, overflow: TextOverflow.fade),
          const SizedBox(height: 30),
          const Text('EmailField: '),
          EmailField(
            onFocusChange: (isFocus) => debugPrint('EmailField isFocus：$isFocus'),
            onValueChange: (email) => debugPrint('EmailField input value：$email'),
          ),
          const SizedBox(height: 30),
          const Text('PasswordField: '),
          PasswordField(
            onFocusChange: (isFocus) => debugPrint('PasswordField isFocus：$isFocus'),
            onValueChange: (password) => setState(() {
              debugPrint('PasswordField input value：$password');
              passwordFieldValue = password;
            }),
            errorMessage: passwordFieldValue.isNotEmpty && !passwordFieldValue.isValidPwd()
  ...
  void doLogin() async {
    final res = await HttpUtils.instance
        .post(Api.login, params: {"mobile_code": "886", "mobile": "0955555555", "pwd": "Aa123456"});
    if (res.isSuccess) {
